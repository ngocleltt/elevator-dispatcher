...
import { boardDestination } from "../elevator/boarding.js";
...
export function renderBuilding() {
    ...
    state.elevators.forEach(elevator => {
        ...
        car.style.top =
            ((state.floors - elevator.floor) * FLOOR_HEIGHT) + "px";
        ...
        display.textContent =
            String(elevator.floor).padStart(2, "0");
    ...
    const boardingPanels =
        document.getElementById("boardingPanels");
    const waitingElevators =
        state.elevators.filter(
            elevator => elevator.boardingAt !== null
        );
    boardingPanels.innerHTML =
        waitingElevators
            .map(elevator => {
                let buttons = "";
                for (
                    let floor = state.floors;
                    floor >= 1;
                    floor--
                ) {
                    const disabled =
            ...
            .join("");
    document
        .querySelectorAll(".floor-pick:not(:disabled)")
        .forEach(button => {
            button.addEventListener(
                "click",
                () => {
                    const elevatorId =
                        Number(button.dataset.elevator);
                    const floor =
    ...
    for (
        ...
        const upButton =
            document.getElementById(`up-${floor}`);
        const downButton =
            document.getElementById(`down-${floor}`);
        const badge =
            document.getElementById(`badge-${floor}`);
        if (upButton) {
        ...
        Object.values(state.hallCalls)
            .forEach(call => {
                ...
                assignedColor =
                    state.elevators[call.elevatorId].color;
                assignedIndex =
                    call.elevatorId + 1;
            });
        if (badge) {
